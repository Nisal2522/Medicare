...
export class NotificationDispatcherService {
  ...
  async onBookingConfirmation(payload: {
    ...
    const patientHtml = this.wrapHtml(
      'Booking confirmed',
    ...
    if (payload.doctorEmail?.trim()) {
      const doctorHtml = this.wrapHtml(
        'New booking',
        `<p>Hello <strong>${this.esc(a.doctorName)}</strong>,</p>
      ...
      await this.mail.sendHtml(
        payload.doctorEmail.trim(),
        `MediSmart — New patient booking (${a.id ?? '—'})`,
        doctorHtml,
      );
    } else {
    ...
    await this.sms.send(
      payload.doctorPhone,
      `MediSmart: New booking ${a.id ?? ''} — ${a.patientName ?? 'Patient'} @ ${summary}.`,
    );
  }
  ...
  async onVideoReminder(payload: {
    ...
    if (payload.doctorEmail?.trim()) {
      await this.mail.sendHtml(
        payload.doctorEmail.trim(),
        `MediSmart — Upcoming video visit (${a.id ?? '—'})`,
        this.wrapHtml(
          'Reminder',
          `<p>Your consultation with <strong>${this.esc(a.patientName)}</strong> is in ~10 minutes.</p>
    ...
    await this.sms.send(
      payload.doctorPhone,
      `MediSmart REMINDER: Consult ${a.patientName ?? 'patient'} @ ${summary} (ID ${a.id ?? ''}).`,
    );
  }
  ...
  async onPrescriptionReady(payload: {
    ...
    const summary =
      typeof rx.medicinesSummary === 'string'
        ? rx.medicinesSummary
  ...
  private formatSlot(a: AppointmentSlice): string {
    const parts = [a.appointmentDateKey, a.day, a.startTime, a.endTime].filter(
      Boolean,
    );
    return parts.join(' · ') || '—';
  }
  ...
  private wrapHtml(title: string, inner: string): string {
    return `<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8"/>
